...
import { DAM_STATS, DAM_CAMERAS, DAM_EVENTS, DAM_ALERTS, DAM_BORDER_CROSSINGS } from '@/lib/dam'
...
export default function RDFBorderOps() {
  ...
  const [cameras, setCameras] = useState<CameraNode[]>(
    DAM_CAMERAS.filter(c => c.institution === 'RDF')
  )
  const [events, setEvents] = useState<IntelligenceEvent[]>(
    DAM_EVENTS.filter(ev => ev.source_tag === 'CCTV_NODE')
  )
  ...
  useEffect(() => {
    Promise.all([
      ...
      if ((c.data as CameraNode[])?.length) {
        setCameras((c.data as CameraNode[]).filter(n => n.institution === 'RDF'))
      }
      if (e.data?.length) {
  ...
  return (
    <div className="space-y-6">
          ...
          return (
            <div key={post.code}
              className={clsx(
                ...
                postOnline === postCams.length && postCams.length > 0
                  ? 'border-green-900/40 bg-green-950/10'
                  : postCams.length === 0 || postOnline === 0
      ...
      {/* Border crossing trend */}
      <div className="rounded-xl border border-rdf/20 bg-slate-900 p-5">
        <h2 className="mb-4 text-sm font-semibold text-slate-200">Border Crossings — Last 7 Days</h2>
        <ResponsiveContainer width="100%" height={200}>
          <ComposedChart data={DAM_BORDER_CROSSINGS} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
            <defs>
              <linearGradient id="crossingsGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <Area yAxisId="left" type="monotone" dataKey="crossings" stroke="#15803D" fill="url(#crossingsGrad)"
              strokeWidth={2} name="Total Crossings" />
            <Bar yAxisId="right" dataKey="alerts" fill="#DC2626" radius={[3, 3, 0, 0]} name="Alerts" />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
