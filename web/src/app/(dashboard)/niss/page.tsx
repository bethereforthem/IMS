...
import { statsApi, siemApi } from '@/lib/api'
...
import {
  ...
  Activity, Globe, Lock, CheckCircle, Clock, XCircle,
} from 'lucide-react'
import { formatDistanceToNow } from 'date-fns'
import clsx from 'clsx'
import type { DashboardStats, IntelligenceEvent, SiemEvent } from '@/types'
import {
  DAM_STATS, DAM_EVENTS, DAM_SIEM, DAM_ALERTS,
  DAM_TREND, DAM_PARTNERS,
} from '@/lib/dam'
...
export default function NISSCommandCenter() {
  ...
  const [siem, setSiem] = useState<SiemEvent[]>(DAM_SIEM)
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-xl border border-slate-800 bg-slate-900 p-5">
          ...
          <AlertFeed limit={5} initialAlerts={DAM_ALERTS} />
        </div>
            ...
            {DAM_PARTNERS.map(p => (
              <div key={p.code}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500">MOU expires {p.mou_expires}</p>
                </div>
                <div className="text-right shrink-0">
                  <span className="text-xs font-bold text-green-400">{p.status}</span>
                  <p className="text-[11px] text-slate-500">{p.recent_queries} queries</p>
                </div>
              </div>
            ))}
