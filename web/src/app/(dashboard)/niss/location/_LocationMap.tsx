...
export default function LocationMap({ locations, agents = [], onDirectAgent }: Props) {
  ...
  const registeredRef   = useRef<Set<string>>(new Set()) // all alarm IDs we registered
  ...
  // ── SOS agent tracking — alarms immediately, even on first render ──────────
  useEffect(() => {
  ...
  // ── Location alarm tracking — skips initial load data ─────────────────────
  useEffect(() => {
  ...
  // ── Drop all registered alarms on unmount ─────────────────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    const handler = () => {
      ...
      setTimeout(() => {
        if (mapRef.current) (mapRef.current as { invalidateSize: () => void }).invalidateSize()
      }, 150)
    }
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      const map = L.map(divRef.current, {
        ...
        zoomSnap: 0.5, zoomDelta: 0.5, wheelPxPerZoomLevel: 100, zoomAnimationThreshold: 4,
      })
      mapRef.current = map
      ...
      agents.forEach((agent) => {
        ...
        marker.bindPopup(`
          <div style="min-width:220px;font-size:12px;font-family:'Courier New',monospace;padding:2px">
            ...
            ${onDirectAgent ? `<button id="direct-${agent.id}" style="width:100%;padding:7px;background:${isSOS ? '#dc2626' : '#3b82f6'};color:white;border:none;cursor:pointer;font-size:11px;font-weight:bold;border-radius:4px;letter-spacing:0.5px;font-family:'Courier New',monospace;">📡 DIRECT THIS AGENT</button>` : ''}
          </div>
        `, { maxWidth: 270 })
        ...
        marker.on('popupopen', () => {
          ...
          if (btn && onDirectAgent) {
            btn.addEventListener('click', () => { map.closePopup(); onDirectAgent(agent) })
          }
        })
      ...
      L.control.layers(
        baseLayers,
        { '🎯 Suspect Locations': suspectLayer, '📡 Field Agents GPS': agentLayer },
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', height: '100%', width: '100%' }}>
      ...
      {/* ── STOP ALARM — top-center, pulsing red ─────────────────────────── */}
      {alarm.active && (
        <div style={{
      ...
      <button
        ...
        style={{
          position: 'absolute', bottom: 40, right: 10, zIndex: 1001,
