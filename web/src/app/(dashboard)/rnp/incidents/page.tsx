...
import type { PatrolZone } from './_IncidentMap'

const RNPIncidentMap = dynamic(() => import('./_IncidentMap'), { ssr: false })

// ── Patrol zones / police stations shown on the map ───────────────────────────

const PATROL_ZONES: PatrolZone[] = [
  { name: 'Nyarugenge Police',  district: 'Kigali',   code: 'NYR', lat: -1.9536, lng: 30.0606 },
  { name: 'Gasabo Police',      district: 'Kigali',   code: 'GSB', lat: -1.8904, lng: 30.1044 },
  { name: 'Kicukiro Police',    district: 'Kigali',   code: 'KCK', lat: -2.0143, lng: 30.0899 },
  { name: 'Huye Police',        district: 'Huye',     code: 'HUY', lat: -2.5965, lng: 29.7397 },
  { name: 'Musanze Police',     district: 'Musanze',  code: 'MSZ', lat: -1.4980, lng: 29.6337 },
  { name: 'Rubavu Police',      district: 'Rubavu',   code: 'RBV', lat: -1.6763, lng: 29.3460 },
  { name: 'Muhanga Police',     district: 'Muhanga',  code: 'MHG', lat: -2.0835, lng: 29.7434 },
  { name: 'Kayonza Police',     district: 'Kayonza',  code: 'KYZ', lat: -1.8773, lng: 30.5992 },
]
...
const ASSIGN_INSTITUTIONS = ['RNP', 'RDF', 'NISS'] as const
...
function IncidentModal({
  ...
  const instStyle: Record<string, string> = {
    RNP:  'bg-blue-700 border-blue-500 text-white shadow-lg shadow-blue-900/50',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-16 bg-black/70 backdrop-blur-sm overflow-y-auto">
      <div className="w-full max-w-2xl rounded-2xl border border-blue-900/60 bg-slate-900 shadow-2xl shadow-blue-900/20">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[9px] font-bold text-blue-400 tracking-widest uppercase">RNP Field Report</span>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-800/40 px-4 py-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold text-white">{report.agent_name ?? 'Unknown Officer'}</p>
              ...
              {showHistory && (
                <div className="rounded-lg border border-slate-700 bg-slate-800/40 px-4 py-3 space-y-1 text-[11px] text-slate-400">
                  ...
                  <p>Status: <span className={clsx('font-bold',
                    report.tracking_session.status === 'ACTIVE' ? 'text-blue-400' :
          ...
          <div className="border-t border-slate-800 pt-5">
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-blue-700 hover:bg-blue-600 disabled:opacity-50 text-white text-sm font-bold py-2.5 rounded-xl transition-colors flex items-center justify-center gap-2"
            >
...
function ReportRow({ report, onClick }: { report: WebFieldReport; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left rounded-xl border border-slate-800 bg-slate-900/70 p-4 hover:border-blue-800 hover:bg-slate-900 transition-colors"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {hasCoords && <MapPin className="h-3 w-3 text-blue-400 shrink-0" />}
          </div>
...
export default function RNPIncidentsPage() {
  ...
  const activeOfficers = agents.filter(a => a.session_status === 'ACTIVE').length
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">RNP Field Incidents</h1>
          <p className="text-xs text-slate-400 mt-0.5">
            Real-time map of officer reports and live GPS tracking — RNP only
          </p>
        </div>
        <button
          onClick={() => load(true)}
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-blue-800 bg-blue-900/20 px-3 py-1.5">
          <Radio className="h-3 w-3 text-blue-400" />
          <span className="text-xs text-blue-300 font-semibold">{activeOfficers} Tracking</span>
        </div>
        ...
        <div className="flex items-center gap-2 rounded-lg border border-blue-900 bg-blue-950/40 px-3 py-1.5">
          <Shield className="h-3 w-3 text-blue-600" />
          <span className="text-xs text-blue-500 font-semibold">{PATROL_ZONES.length} Patrol Zones</span>
        </div>
      </div>

      {/* Map */}
      <div className="rounded-2xl border border-blue-900/40 overflow-hidden h-[500px] relative">
        {loading && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-slate-950/80">
            <div className="flex flex-col items-center gap-3">
              <div className="h-6 w-6 animate-spin rounded-full border-2 border-blue-500 border-t-transparent" />
              <span className="text-xs text-slate-400">Loading RNP incidents…</span>
            </div>
          </div>
        )}
        <RNPIncidentMap
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-12 text-center">
          ...
          <p className="text-sm text-slate-400">No RNP incident reports match the current filters.</p>
        </div>
      ) : (
