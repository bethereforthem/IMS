...
type InstitutionFilter = 'ALL' | 'RIB' | 'RNP' | 'NISS' | 'RDF' | 'RCS'
...
const INSTITUTION_FILTERS: InstitutionFilter[] = ['ALL', 'RIB', 'RNP', 'NISS', 'RDF', 'RCS']
...
export default function RNPCasesPage() {
  const { user } = useAuth()
  ...
  const filtered = cases.filter((c) => {
    ...
    if (institutionFilter !== 'ALL' && c.lead_institution !== institutionFilter) return false
    return true
  })

  const underInvestigation = cases.filter((c) => c.status === 'UNDER_INVESTIGATION').length
  const prosecution = cases.filter((c) => c.status === 'PROSECUTION').length
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">NATIONAL CASES</h1>
          <p className="text-sm text-slate-400 mt-0.5">
            {user?.full_name} · {user?.clearance_level} clearance · Cross-institution view
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'In Prosecution', value: prosecution, icon: Scale, cls: 'text-orange-400' },
          { label: 'Closed', value: closed, icon: CheckCircle, cls: 'text-green-400' },
        ].map((s) => (
          ...
          {INSTITUTION_FILTERS.map((f) => (
            <button
              ...
              className={clsx(
                ...
                institutionFilter === f ? 'bg-rnp text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'
              )}
            >
              {f}
            </button>
        ...
        <div className="flex flex-wrap gap-1.5">
          {STATUS_FILTERS.map((f) => (
            <button
              ...
              className={clsx(
                'text-xs px-2.5 py-1 rounded-lg font-medium transition-colors',
                statusFilter === f ? 'bg-rnp text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'
              )}
            >
              {f.replace(/_/g, ' ')}
            </button>
        ...
        <div className="flex flex-wrap gap-1.5">
          {CLASS_FILTERS.map((f) => (
            <button
              ...
              className={clsx(
                'text-xs px-2.5 py-1 rounded-lg font-medium transition-colors',
                classFilter === f ? 'bg-rnp text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'
              )}
            >
              {f.replace('_', ' ')}
            </button>
                ...
                return (
                  <tr key={c.id} className="border-b border-slate-800/50 text-xs hover:bg-slate-800/20">
                    <td className="px-4 py-3">
                      <span className="font-mono text-rnp">{c.case_reference}</span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="flex items-center gap-1 text-[10px] font-semibold text-rnp border border-rnp/30 rounded px-2 py-0.5 hover:bg-rnp/10 transition-colors"
                      >
