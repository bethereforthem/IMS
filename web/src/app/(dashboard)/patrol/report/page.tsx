...
export default function PatrolReportPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!insecurityType) { setFormError('Please select a type of insecurity.'); return }
    if (!description.trim()) { setFormError('Please describe the incident.'); return }
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-green-800/40 bg-green-950/10 p-8 text-center space-y-4 max-w-md">
          ...
          <button
            onClick={() => {
              setSuccess(false)
              setPersonName(''); setInsecurityType(''); setLocationDesc('')
      ...
      <form onSubmit={handleSubmit} className="space-y-5 max-w-lg">
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-400 mb-2 uppercase tracking-wide">
            Type of Insecurity <span className="text-red-400">*</span>
          </label>
          <select
            value={insecurityType}
            onChange={e => setInsecurityType(e.target.value)}
            className="w-full bg-slate-800 border border-slate-700 focus:border-patrol/50 rounded-lg px-3 py-2.5 text-sm text-slate-200 outline-none transition-colors"
          >
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-400 mb-2 uppercase tracking-wide">
            Person Name <span className="text-slate-600">(if known)</span>
          </label>
          <input
            value={personName}
            onChange={e => setPersonName(e.target.value)}
            placeholder="Full name or alias"
            className="w-full bg-slate-800 border border-slate-700 focus:border-patrol/50 rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder-slate-500 outline-none transition-colors"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Describe the behaviour, pattern, or incident. Include relevant details such as times, frequency, or witnesses."
            rows={5}
            className="w-full bg-slate-800 border border-slate-700 focus:border-patrol/50 rounded-lg px-3 py-2.5 text-sm text-slate-200 placeholder-slate-500 outline-none transition-colors resize-none"
          />
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-400 mb-2 uppercase tracking-wide">
            Exhibits <span className="text-slate-600">(optional — images or video)</span>
          </label>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
          <div className="flex items-start gap-2">
            ...
            <p className="text-[10px] text-slate-500 leading-relaxed">
              This report will be forwarded to RNP Command and logged in the national intelligence system.
